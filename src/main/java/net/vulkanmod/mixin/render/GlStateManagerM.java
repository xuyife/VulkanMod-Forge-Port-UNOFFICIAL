...
public class GlStateManagerM {
     ...
     */
    @Overwrite(remap = false)
    public static void _upload(int pLevel, int pXOffset, int pYOffset, int pWidth, int pHeight, NativeImage.Format pFormat, IntBuffer pPixels, Consumer<IntBuffer> pOutput) {
        try {
            RenderSystem.assertOnRenderThreadOrInit();
            ...
            GlTexture.texSubImage2D(3553, pLevel, pXOffset, pYOffset, pWidth, pHeight, pFormat.glFormat(), 5121, MemoryUtil.memByteBuffer(pPixels));
        } finally {
